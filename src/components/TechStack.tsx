...
const createCenteredLogoTexture = (url: string) => {
  const size = 1024;
  ...
  if (!ctx) {
    const fallback = textureLoader.load(url);
    fallback.colorSpace = THREE.SRGBColorSpace;
    return fallback;
  }

  // Base white tile so logo remains visible even with transparent assets.
...
const textures = imageUrls.map((url) => createCenteredLogoTexture(url));
...
const TechStack = () => {
  ...
  const [sphereCount, setSphereCount] = useState(
    window.matchMedia("(max-width: 900px)").matches ? 18 : 30
  );

  useEffect(() => {
    const handleViewportChange = () => {
      setSphereCount(window.matchMedia("(max-width: 900px)").matches ? 18 : 30);
    };
